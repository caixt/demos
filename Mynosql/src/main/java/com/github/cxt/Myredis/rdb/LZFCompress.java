...
public final class LZFCompress {
	...
	public static int compress(byte[] in, int inLen, byte[] out, int outPos) {
		...
		while (inPos < inLen - 4) {
			...
			int ref = hashTab[off];
			hashTab[off] = inPos;
			if (ref < inPos && ref > 0 && (off = inPos - ref - 1) < MAX_OFF
					&& in[ref + 2] == p2 && in[ref + 1] == (byte) (future >> 8)
	 ...
	 */
	public static void expand(byte[] in, int inPos, int inLen, byte[] out,
			int outPos, int outLen) {
