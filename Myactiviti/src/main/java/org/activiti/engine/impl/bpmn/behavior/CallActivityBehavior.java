...
public class CallActivityBehavior extends AbstractBpmnActivityBehavior implements SubProcessActivityBehavior {
  ...
  public void execute(ActivityExecution execution) throws Exception {
    ...
    //增加默认输入为所有的父的参数
    Map<String, Object> allparms = execution.getVariables();
    for(String key : allparms.keySet()){
    	subProcessInstance.setVariable(key, allparms.get(key));
    }
  ...
  public void completing(DelegateExecution execution, DelegateExecution subProcessInstance) throws Exception {
	//增加默认输出为所有的子的参数
    Map<String, Object> allparms = subProcessInstance.getVariables();
    for(String key : allparms.keySet()){
    	execution.setVariable(key, allparms.get(key));
    }  
